...
def load_cnn_virus():
    ...
    dataframe = pandas.read_csv('1train.csv')
    ...
    zeros = np.zeros([features.shape[0],14], dtype=float)
    print(zeros)
    ...
    features = features.reshape(-1,1,22,22) #transfer to a image
...
def start_websocket_server_worker(id, host, port, hook, verbose, keep_labels=None, training=True):
    ...
    if training:
        ...
        '''
        selected_data = d[0].to(device)
        selected_targets=d[1].to(device)
