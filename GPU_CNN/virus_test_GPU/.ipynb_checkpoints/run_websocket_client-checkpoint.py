...
def loss_fn(pred, target):
    return F.nll_loss(input=pred.to("cpu"), target=target.to("cpu"))
...
class Net(nn.Module):
    def __init__(self):
        super(Net,self).__init__()
        self.conv1 = nn.Conv2d(1, 50, kernel_size=3,padding=2)
        self.conv2 = nn.Conv2d(50, 40, kernel_size=3,padding=2)
        self.conv3 = nn.Conv2d(40, 30, kernel_size=3)        
        self.mp = nn.MaxPool2d(2)
        self.relu = nn.ReLU()
        self.fc1 = nn.Linear(120,120)  # 必须为16*5*5
